...
import { getShowById, getSeatMatrix, lockSeats } from "../api/backend";
...
// Helper to generate dynamic occupied seats based on date/time
const generateOccupiedSeats = (movieId, date, time) => {
  let hash = 0;
  const str = `${movieId}-${date}-${time}`;
  ...
  const numOccupied = (seed % 15) + 5; // 5 to 20 seats occupied
  
  for (let i = 0; i < numOccupied; i++) {
    const row = rows[(seed + i * 3) % rows.length];
    const seatNum = ((seed + i * 7) % 12) + 1;
...
export default function SeatSelection() {
  ...
  const { data: seatMatrix, isLoading: seatsLoading } = useQuery({
    queryKey: ["seats", showId],
    queryFn: () => getSeatMatrix(showId),
    enabled: !!showId,
  });
  ...
  useEffect(() => {
    ...
    client.onConnect = () => {
      console.log("Connected to WebSocket for show", showId);
      client.subscribe(`/topic/show/${showId}`, (message) => {
  ...
  // Clear selected seats if date/time changes
  useEffect(() => {
    setSelectedSeats([]);
  ...
  const toggleSeat = (seatObj, cat) => {
    if (seatObj.seatStatus !== "AVAILABLE") return;
    const id = seatObj.id; // Backend seat ID
    const seatIdStr = seatObj.seatNumber; // e.g. "A-1"
    ...
    setSelectedSeats(prev => {
      ...
      const [rowLabel, seatNumStr] = seatIdStr.split("-");
      const clickedNum = parseInt(seatNumStr, 10);
      // Helper to find seat by seatNumber
      const findSeat = (targetIdStr) => {
        for (let row of seatMatrix) {
          for (let seat of row) {
            if (seat && seat.seatNumber === targetIdStr) return seat;
          }
        }
        return null;
      };
      
      // Determine how many seats we need to select.
      let seatsToSelect = seatQuantity;
      let baseSelection = [];
      
      if (prev.length > 0 && prev.length < seatQuantity) {
        seatsToSelect = seatQuantity - prev.length;
        baseSelection = [...prev];
      }
      
      let newSelection = [];
      
      // Try to select to the right first
      let count = 0;
      let checkNum = clickedNum;
      
      while (count < seatsToSelect) {
        const idToCheck = `${rowLabel}-${checkNum}`;
        const seatCheck = findSeat(idToCheck);
        
        if (!seatCheck) break; // End of row
        if (seatCheck.seatStatus !== "AVAILABLE") break; // Hit occupied
        if (baseSelection.find(s => s.id === seatCheck.id)) break; // Hit already selected
        
        newSelection.push({ id: seatCheck.id, seatNumber: seatCheck.seatNumber, price: cat.price });
        count++;
        checkNum++;
      }
      
      // Try going left if we need more seats
      if (newSelection.length < seatsToSelect) {
         let leftNum = clickedNum - 1;
         while (newSelection.length < seatsToSelect && leftNum >= 1) {
            const idToCheck = `${rowLabel}-${leftNum}`;
            const seatCheck = findSeat(idToCheck);
            
            if (!seatCheck) break;
            if (seatCheck.seatStatus !== "AVAILABLE") break;
            if (baseSelection.find(s => s.id === seatCheck.id)) break;
            
            newSelection.unshift({ id: seatCheck.id, seatNumber: seatCheck.seatNumber, price: cat.price }); 
            leftNum--;
         }
      }
      
      return [...baseSelection, ...newSelection];
    });
  };
  ...
  if (showLoading || movieLoading || seatsLoading) return <div className="min-h-screen flex items-center justify-center bg-[#080810]"><div className="w-8 h-8 rounded-full border-2 border-white/20 border-t-[#e11d48] animate-spin" /></div>;
  if (!movie || !showData || !seatMatrix) return null;
                    ...
                    return (
                      <div key={category.id} className="flex flex-col gap-3 relative border-b border-white/5 pb-8 last:border-0">
                        ...
                        {categoryRows.map(rowLabel => {
                          // Find the row array in seatMatrix
                          const rowObj = seatMatrix.find(r => r.length > 0 && r.find(s => s && s.rowLabel === rowLabel));
                                  ...
                                  return (
                                    <div key={id} className="relative group">
                                      <motion.button
                                        ...
                                        onClick={() => toggleSeat(seat, category)}
                                        className={`
          ...
          >
            ...
            >
              ...
              <button
                ...
                className="w-full py-5 bg-green-500 text-white font-bold text-lg hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/30"
              >
                Select Seats
              </button>
            </motion.div>
